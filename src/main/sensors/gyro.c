...
{
    return calibratingG == (CALIBRATING_GYRO_CYCLES / targetLooptime) * 1000;
}
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (isOnFinalGyroCalibrationCycle()) {
            ...
            if (gyroMovementCalibrationThreshold && dev > gyroMovementCalibrationThreshold) {
                gyroSetCalibrationCycles((CALIBRATING_GYRO_CYCLES / targetLooptime) * 1000);
                return;
            }
            gyroZero[axis] = (g[axis] + ((CALIBRATING_GYRO_CYCLES / targetLooptime) * 500)) / (CALIBRATING_GYRO_CYCLES / targetLooptime) * 1000;
        }
    }
